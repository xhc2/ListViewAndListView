...
/**
 * Created by pc on 2016/5/26. 解决listview，和expandlistview同屏的问题
 */
...
//                    invalidate(); // 移动完后刷新界面
    ...
    public boolean isTop() {
        // Log.e("xhc", "mylist is top "+getScrollY());
        boolean flag = false;
        if (listView.isShown()) {
            flag = listViewIsTop();
        } else {
            flag = expandListViewIsTop();
        }

        return getScrollY() <= 0 && flag;
    }
    ...
    private boolean listViewIsTop() {
        ...
        // listView.get
        // Log.e("xhc", " listview is top---> "+firstVisblePosition+" y ");
        if (firstVisblePosition <= 0) {
            return true;
        }
        return false;
    }
